...
◯pagecontentの画像URLの保存方法について：新しい画像がない保存でも、既存画像を消さないため（書かないとnullになり、本文保存時に画像URLが上書きされて消えるから）
・画像を選ばずに本文だけ保存
→ DBの既存 imageUrl をそのまま保存する
→ 画像は消えない

・新しい画像を選んで保存
→ Supabaseにアップロードする
→ 返ってきたURLで imageUrl を更新する

・画像削除ボタンを押す
→ deletePageContentImage で imageUrl を null にする

◯工夫した点
・pagecontentのブロックキーをconstantsにまとめて定義化して各ページには型チェック関数で名称が定義と同じかをチェックした
・pagecontentのfallbackを一つのファイルにまとめることで変更しやすくした
・データ新規保存、更新、削除、並び順変更時にトライ・キャッチで必要であればエラーを返すことで、エラーで500になるのを防ぐ（問い合わせ/体験・見学ステータス変更、FAQ並び順変更、練習スケジュール変更、pagecontent変更、メール通知先変更）
・フォーム送信エラーの際にページトップ（「入力内容を確認してください」というエラー表示が見える位置）に遷移させることでユーザーにエラーだとわかるようにした
・モーダルやドロワーをすべてEscで閉じれるようにした
・ラインは通知、管理画面は詳細確認と役割を分けた。具体的には、ラインの通知は一目に触れやすいため、「名前」「参加内容」「第一希望日」など必要最低限の情報しか載せない用にして、個人情報やセンシティブな内容が外部に漏れないようにした。



◯管理者ページのログインフォーム送信の流れ
【自分の言葉で書く】
1. Server Action が FormData を受け取る
2. ブロック判定を行う：送信失敗数判定関数にFormDataを入れてDBを確認し、同一IP：3分間に5回以上の試行でブロック・同一メール：3分間に3回以上の失敗でブロックしたらエラー（BLOCKED）を返して、DBテーブル（LoginFormSubmissionLog）に記録した後にエラーを返す（短時間に複数回送信されています。少し時間をおいてから再度お試しください）これにより、3回失敗したらいっときsupabase Authへログイン申請の送信自体をできなくする。
3. supabase AuthでメアドとPWがあってるかの確認：supabase AuthでメアドとPWがあってる確認する。4. DBに記録する：あってるならDBテーブルに成功を記録してログインし、間違ってるなら、DBテーブルに失敗を記録して「このエラーはメールアドレスまたはパスワードが正しくありません」を返す。
【簡潔に書く】
1. Server ActionがFormDataを受け取る
2. email/passwordの空チェック
3. IP/UserAgentを取得
4. IP/emailをハッシュ化してログイン制限チェック
5. 制限超過なら Supabase Auth に送らず BLOCKED ログ保存
6. 制限内なら Supabase Auth に送る
7. 失敗なら FAILED ログ保存
8. 成功なら SUCCESS ログ保存
9. /admin/dashboard へ redirect


・NEXT_PUBLIC_ が付いた環境変数
＝ブラウザに渡してもいい公開情報
・SUPABASE_SERVICE_ROLE_KEY
＝サーバーだけが持つ秘密の管理者鍵
故に、SUPABASE_SERVICE_ROLE_KEYにNEXT_PUBLIC_ は絶対につけない
※SUPABASE_SERVICE_ROLE_KEYが漏れると、
DBのデータを読まれる
DBのデータを書き換えられる
Storageのファイルを消される
管理者用の操作をされる
...
・FaqCategoryValue と FaqCategoryについて
FaqCategoryValue
→ URLクエリ・select・画面の絞り込みで使う「画面側のカテゴリ値」

FaqCategory
→ Prisma/DBの faq.category に保存されている「DB側のカテゴリ値」

◯imageUrlの立ち位置
parsePageContentFormData()
→ pageKey / blockKey / content / imageAlt だけを成功データとして返す

imageUrl
→ フォームからは信用しない
→ DBまたはStorageアップロード結果から決める

values.imageUrl
→ エラー時の画面復元用としてだけ残す
ページコンテント変更画面で画像を選択して保存した場合、
通常のフォームバリデーションに失敗したら、本文などの入力値はstateで戻す。
ただし、選択したアップロード画像自体は保持できないため、画像プレビューは既存の保存済み画像を表示する。

バリデーション成功後は、DBから現在保存済みの imageUrl / imagePath を取得する。
画像形式エラーや5MB超過エラーの場合はDB保存せず、既存画像プレビューを維持するために保存済み画像URLを返す。

画像チェックを通過してアップロードに成功した場合は、新しい imageUrl / imagePath をDB保存に使う。
DB保存に成功したら新しい画像が表示され、古い画像はStorageから削除する。
DB保存に失敗した場合は、アップロード済みの新画像を削除し、画面には既存の保存済み画像を表示する。

保存に成功するまでは、既存画像を正として扱う。
新画像は「Storageアップロード成功 + DB保存成功」して初めて正式採用。


◯なぜ画像Urlだけバリデーションで他のデータとは別に扱わなくてはいけないの？バリデーションの中ではなくアクション関数の中でバリデーションを行う理由は？
ーimageUrl は「ユーザーが入力して保存する値」ではなく、「サーバー側が作って管理する値」だから、他の入力値と同じ扱いにしない方がいい。画像Urlは画像ファイルをStorageにアップロードした結果、サーバー側で決まるURL。だからフォームから送られてくる画像Urlは信用しないほうがいい。